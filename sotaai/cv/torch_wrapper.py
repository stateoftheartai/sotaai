...
def load_dataset(dataset_name, root='default',
                 ann_file=None,
    ...
    if dataset_name in datasets_w_train:
        ds_dic['train'] = torch.utils.data.DataLoader(
            ds(root+"/train",
               train=True,
            ...
            shuffle=False,
            num_workers=0)
        ds_dic['test'] = torch.utils.data.DataLoader(
            ds(root+"/test",
                train=False,
        ...
        if dataset_name == 'SVHN':
            ds_dic['extra_training_set'] = ds(
                root+"/extra", split="extra", download=True)
        elif dataset_name == 'STL10':
            ds_dic['unlabeled'] = ds(
                root+"/extra", split="unlabeled", download=True)
        ...
        ds_dic['train'] = ds(root+"/train", split='balanced',
                             train=True, download=download_train)
        ds_dic['test'] = ds(root+"/test", split='balanced',
                            train=False, download=download_test)

    elif "PhotoTour" in dataset_name:

        ds_dic['train'] = ds(root+"/train", name=name,
                             download=download_train, train=True)
        ds_dic['test'] = ds(root+"/test", name=name,
                            download=download_test, train=False)

    elif dataset_name == 'Omniglot':
        download_bg = not os.path.exists(root + "/background")
        download_eval = not os.path.exists(root + "/eval")
        ds_dic['background'] = ds(
            root + "/background", background=True, download=download_bg)
        ds_dic['evaluation'] = ds(
            root + "eval", background=False, download=download_eval)

    elif dataset_name in ["SBU", 'SEMEION']:
        ds_dic['data'] = ds(root, download=True)

    elif dataset_name in ["VOCSegmentation", "VOCDetection"]:
        ds_dic['train'] = ds(root+"/train", year=year,
                             image_set="train", download=download_train)
        download_val = not os.path.exists(root+"/val")
        ds_dic['val'] = ds(root+"/val", year=year,
                           image_set="val", download=download_val)
    ...
    elif dataset_name in ['CocoDetection',
                          'CocoCaptions',
                          'Flickr8k',
                          'Flickr30k']:
        ...
        ds_dic['train'] = ds(root + 'train', ann_file,
                             frames_per_clip, train=True)
        ds_dic['test'] = ds(root + 'test', ann_file,
                            frames_per_clip, train=False)
